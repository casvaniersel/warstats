...
export default function WSTournament({ tournament }: IWSTournamentProps) {
    ...
    const getRuleTypeIcon = (): Array<TournamentType> => {
        switch (tournament.rules.type) {
            case RuleType.Kills:
                return [{ icon: faSkull, ruleType: RuleType.Kills }];
            case RuleType.KillsAndPlacement:
                return [
                    { icon: faSkull, ruleType: RuleType.Kills },
                    { icon: faAward, ruleType: RuleType.Placement },
                ];
            case RuleType.Placement:
                return [{ icon: faAward, ruleType: RuleType.Placement }];
            case RuleType.Bracket:
                return [{ icon: faBomb, ruleType: RuleType.Bracket }];
        }
    };
                    ...
                    <Grid item xs={12} className={classes.homeContainer}>
                        <Card className={classes.tournamentInfoCard}>
                            <div
                                ...
                                onClick={openTournamentTypeDialog}>
                                {getRuleTypeIcon().map((tournamentType, i) => (
                                    <FontAwesomeIcon
                                        icon={tournamentType.icon}
                                        key={i}
                                ...
                                <div className={classes.timeSpan}>
                                    <div className={classes.calendar}>
                                        <div className={classes.calendarHeader}>
                                            {formatDate(tournament.start).month}
                                        </div>
                                        <div className={classes.calendarDate}>
                                            {formatDate(tournament.start).day}
                                        </div>
                                    </div>
                                    <div
                                        className={
